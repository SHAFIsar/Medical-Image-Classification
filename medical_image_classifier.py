...
def train_model():
    ...
    val_gen = val_datagen.flow_from_directory("data/val", target_size=IMG_SIZE, batch_size=BATCH_SIZE, class_mode="categorical")
    num_classes = train_gen.num_classes
    model = build_resnet_model(num_classes)
    history = model.fit(train_gen, validation_data=val_gen, epochs=EPOCHS)
    model.save("medical_model.h5")
    logging.info("Model saved as medical_model.h5")
...
def load_model():
    ...
    if os.path.exists("medical_model.h5"):
        model = tf.keras.models.load_model("medical_model.h5")
        datagen = ImageDataGenerator(rescale=1./255)
        gen = datagen.flow_from_directory("data/train", target_size=IMG_SIZE, batch_size=BATCH_SIZE, class_mode="categorical")
